...
remove(bike_jan, bike_feb, bike_mar, bike_apr, bike_may, bike_jun, bike_jul, bike_aug, bike_sep)
...
remove(age1, age2, age3, age4, age5, age6, age7, age8)
...
remove(ageTeens, ageYoungAdults, ageAdults, ageSeniors)
```
...
remove(genderM, genderF, genderMClean, genderFClean)
```
...
```{r}
ageTeensClean$starttime <- as.POSIXlt(ageTeensClean$starttime, tz = "", "%m/%d/%Y %H:%M:%OS", optional = FALSE)
ageTeensClean$stoptime <- as.POSIXlt(ageTeensClean$stoptime, tz = "", "%m/%d/%Y %H:%M:%OS", optional = FALSE)

ageTeensClean$month <- month(ageTeensClean$starttime) #helps with seasons
ageTeensClean$hour <- hour(ageTeensClean$starttime) #helps with time of day
ageTeensClean$hour <- ifelse((minute(ageTeensClean$starttime) >= 16 & minute(ageTeensClean$starttime) <= 30), ageTeensClean$hour+0.25, ageTeensClean$hour)
ageTeensClean$hour <- ifelse((minute(ageTeensClean$starttime) >= 31 & minute(ageTeensClean$starttime) <= 45), ageTeensClean$hour+0.5, ageTeensClean$hour)
ageTeensClean$hour <- ifelse((minute(ageTeensClean$starttime) >= 46 & minute(ageTeensClean$starttime) <= 60), ageTeensClean$hour+0.75, ageTeensClean$hour)

numTeensHours <- seq(from = 0, to = 23.75, by = 0.25)
numTeensHoursAt <- numeric(length(numTeensHours))
for(i in 1:length(numTeensHours)) {
  numTeensHoursAt[i] = length(which(ageTeensClean$hour == numTeensHours[i])) 
}
...
ageYoungAdultsClean$starttime <- as.POSIXlt(ageYoungAdultsClean$starttime, tz = "", "%m/%d/%Y %H:%M:%OS", optional = FALSE)
ageYoungAdultsClean$stoptime <- as.POSIXlt(ageYoungAdultsClean$stoptime, tz = "", "%m/%d/%Y %H:%M:%OS", optional = FALSE)

ageYoungAdultsClean$month <- month(ageYoungAdultsClean$starttime) #helps with seasons
ageYoungAdultsClean$hour <- hour(ageYoungAdultsClean$starttime) #helps with time of day
ageYoungAdultsClean$hour <- ifelse((minute(ageYoungAdultsClean$starttime) >= 16 & minute(ageYoungAdultsClean$starttime) <= 30), ageYoungAdultsClean$hour+0.25, ageYoungAdults$hour)
ageYoungAdultsClean$hour <- ifelse((minute(ageYoungAdultsClean$starttime) >= 31 & minute(ageYoungAdultsClean$starttime) <= 45), ageYoungAdultsClean$hour+0.5, ageYoungAdults$hour)
ageYoungAdultsClean$hour <- ifelse((minute(ageYoungAdultsClean$starttime) >= 46 & minute(ageYoungAdultsClean$starttime) <= 60), ageYoungAdultsClean$hour+0.75, ageYoungAdultsClean$hour)
...
for(i in 1:length(numYoungAdultsHours)) {
  numYoungAdultsHoursAt[i] = length(which(ageYoungAdultsClean$hour == numYoungAdultsHours[i])) 
}
...
ageAdultsClean$starttime <- as.POSIXlt(ageAdultsClean$starttime, tz = "", "%m/%d/%Y %H:%M:%OS", optional = FALSE)
ageAdultsClean$stoptime <- as.POSIXlt(ageAdultsClean$stoptime, tz = "", "%m/%d/%Y %H:%M:%OS", optional = FALSE)

ageAdultsClean$month <- month(ageAdultsClean$starttime) #helps with seasons
ageAdultsClean$hour <- hour(ageAdultsClean$starttime) #helps with time of day
ageAdultsClean$hour <- ifelse((minute(ageAdultsClean$starttime) >= 16 & minute(ageAdultsClean$starttime) <= 30), ageAdultsClean$hour+0.25, ageAdults$hour)
ageAdultsClean$hour <- ifelse((minute(ageAdultsClean$starttime) >= 31 & minute(ageAdultsClean$starttime) <= 45), ageAdultsClean$hour+0.5, ageAdults$hour)
ageAdultsClean$hour <- ifelse((minute(ageAdultsClean$starttime) >= 46 & minute(ageAdultsClean$starttime) <= 60), ageAdultsClean$hour+0.75, ageAdults$hour)

numAdultHours <- seq(from = 0, to = 23.75, by = 0.25)
numAdultHoursAt <- numeric(length(numAdultHours))
for(i in 1:length(numAdultHours)) {
  numAdultHoursAt[i] = length(which(ageAdultsClean$hour == numAdultHours[i])) 
}
...
ageSeniorsClean$starttime <- as.POSIXlt(ageSeniors$starttime, tz = "", "%m/%d/%Y %H:%M:%OS", optional = FALSE)
ageSeniorsClean$stoptime <- as.POSIXlt(ageSeniors$stoptime, tz = "", "%m/%d/%Y %H:%M:%OS", optional = FALSE)

ageSeniorsClean$month <- month(ageSeniorsClean$starttime) #helps with seasons
ageSeniorsClean$hour <- hour(ageSeniorsClean$starttime) #helps with time of day
ageSeniorsClean$hour <- ifelse((minute(ageSeniorsClean$starttime) >= 16 & minute(ageSeniorsClean$starttime) <= 30), ageSeniorsClean$hour+0.25, ageSeniorsClean$hour)
ageSeniorsClean$hour <- ifelse((minute(ageSeniorsClean$starttime) >= 31 & minute(ageSeniorsClean$starttime) <= 45), ageSeniorsClean$hour+0.5, ageSeniorsClean$hour)
ageSeniorsClean$hour <- ifelse((minute(ageSeniorsClean$starttime) >= 46 & minute(ageSeniorsClean$starttime) <= 60), ageSeniorsClean$hour+0.75, ageSeniorsClean$hour)

numSeniorHours <- seq(from = 0, to = 23.75, by = 0.25)
numSeniorHoursAt <- numeric(length(numSeniorHours))
for(i in 1:length(numSeniorHours)) {
  numSeniorHoursAt[i] = length(which(ageSeniorsClean$hour == numSeniorHours[i])) 
}
...
remove(teens, teensPlot, ageTeensClean, numTeenHours, numTeenHoursAt, youngAdults, youngAdultsPlot, ageYoungAdultsClean, numYoungAdultHours, numYoungAdultsHoursAt, adults, adultsPlot, ageAdultsClean, numAdultHours, numAdultHoursAt, seniors, seniorsPlot, ageSeniorsClean, numSeniorHours, numSeniorHoursAt)
```
...
remove(lbls, colors, pct, slices, malecount, femalecount, malepercent, femalepercent)
```
...
remove(count_clean, bikesgender_clean, bikesgender)
```
...
remove(spring, summer, winter, fall, J)
```
...
remove(bikesbyage_age, bikesbyage_age_by_station)
```
...
ggmap(map1)+geom_point(data = pct_selected_stations , aes(x = bikesample.start.station.longitude, y = bikesample.start.station.latitude,fill = bike_shortage_ratio), size = 2, shape = 21,show.legend = TRUE) + scale_fill_gradient(low = "#0000FF", high = "#FF0000")
```
...
remove(stations_coor, abs_surplus_stations, abs_shortage_stations, abs_selected_stations, pct_surplus_stations, pct_selected_stations, pct_shortage_stations)
```
...
remove(model_customer_snow, newdat_customer_snow, model_subscriber_snow, newdat_subscriber_snow)
```
...
remove(model_customer_PRCP, model_subscriber_PRCP, newdat_subscriber_PRCP, newdat_customer_PRCP)
```
...
remove(model_Teens_snow, newdat_Teens_snow, model_Young_Adults_snow, newdat_Young_Adults_snow, model_Adults_snow, newdat_Adults_snow, model_Seniors_snow, newdat_Seniors_snow)
```
...
remove(model_Teens_PRCP, newdat_Teens_PRCP, model_Young_Adults_PRCP, newdat_Young_Adults_PRCP, model_Adults_PRCP, newdat_Adults_PRCP, model_Seniors_PRCP, newdat_Seniors_PRCP)
```


**From both graphs, we can observe that the older customer base often ride for longer times, and the young adults and seniors group are more sensitive to snow than teens and adult riders.**
**On the other hand, the adults, seniors, and young adults are less sensitive than teens when it comes to rain.**
...
remove(model_customer_TMAX, newdat_customer_TMAX, model_subscriber_TMAX, newdat_subscriber_TMAX)
```
...
remove(model_Teens_TMAX, newdat_Teens_TMAX, model_Young_Adults_TMAX, newdat_Young_Adults_TMAX, model_Adults_TMAX, newdat_Adults_TMAX, model_Seniors_TMAX, newdat_Seniors_TMAX)
```

**Across the age groups, people ride more Citi Bikes as the temperature gets warmer. However, the ridership peaks at approximately 75 degrees. Temperatures hotter than this causes a negative impact on the ride duration. Seniors still have the highest ride duration, followed by adults, young adults, and teens.** 

**However, the senior group's peak ridership temperature arrives sooner than adults, then young adults, and teens. This means the senior group has a lower temperature tolerance that causes the group to ride less with warmer temperatures. Therefore, the seniors are very sensitive to the temperature change, followed by adults, seniors, and teens.**
